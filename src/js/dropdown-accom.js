$(function () {
	...
	const setInput=function () {
		$(this).parentsUntil(dropAcc).find(dropAccInsert)
		.val(`${inputFunc(0)} спален, ${inputFunc('1')} кроватей, ${inputFunc('2')} ванных комнат`)
	}	
	...
	dropAccInput.click(function () {
		...
		if ($(this).siblings(dropAccUl).css('display')==='block') {
			$(this).siblings(dropAccUl).css('display','none')} else
			 {	
				$(this).siblings(dropAccUl).css('display','block')	
			}
		}
	)
